...
const getTracksAudioFeatures: tool<{
  trackIds: z.ZodArray<z.ZodString>;
}> = {
  name: 'getTracksAudioFeatures',
  description:
    'Get audio features (BPM/tempo, energy, danceability, valence) for one or more Spotify tracks. Useful for DJ analysis and genre classification. Max 100 track IDs per request.',
  schema: {
    trackIds: z
      .array(z.string())
      .min(1)
  ...
  handler: async (args, _extra: SpotifyHandlerExtra) => {
    ...
    try {
      ...
      const formatted = items
        .filter(Boolean)
        .map((f) => {
          const key = ['C','C#','D','D#','E','F','F#','G','G#','A','A#','B'][f.key] ?? '?';
          const mode = f.mode === 1 ? 'maj' : 'min';
          return (
            `ID: ${f.id} | BPM: ${Math.round(f.tempo)} | Energy: ${f.energy.toFixed(2)} | ` +
            `Dance: ${f.danceability.toFixed(2)} | Valence: ${f.valence.toFixed(2)} | Key: ${key} ${mode}`
          );
        })
        .join('\n');
      ...
      return {
        content: [
          {
            type: 'text',
            text: `# Audio Features (${items.filter(Boolean).length} tracks)\n\n${formatted}`,
          },
        ],
      };
    } catch (error) {
...
const getArtistsGenres: tool<{
  artistIds: z.ZodArray<z.ZodString>;
}> = {
  name: 'getArtistsGenres',
  description:
    'Get genre tags for one or more Spotify artists. Useful for playlist categorization and DJ set planning. Max 50 artist IDs per request.',
  schema: {
    artistIds: z
      .array(z.string())
      .min(1)
  ...
  handler: async (args, _extra: SpotifyHandlerExtra) => {
    ...
    try {
      const artists = await handleSpotifyRequest(async (spotifyApi) => {
        return await spotifyApi.artists.get(artistIds);
      });

      const items = Array.isArray(artists) ? artists : [artists];
      const formatted = items
        .filter(Boolean)
        .map((a) => {
          const genres = a.genres.length > 0 ? a.genres.join(', ') : 'no genres';
          return `ID: ${a.id} | ${a.name} | Genres: ${genres}`;
        })
        .join('\n');
      ...
      return {
        content: [
          {
            type: 'text',
            text: `# Artist Genres (${items.filter(Boolean).length} artists)\n\n${formatted}`,
          },
        ],
      };
    } catch (error) {
      return {
        content: [
          {
            type: 'text',
            text: `Error fetching artist genres: ${
              error instanceof Error ? error.message : String(error)
            }`,
          },
        ],
